...
def fetch_data(periods):
    ...
    for period in periods:
        query = f"SELECT is_arrival, trajectory_json FROM regional_flights WHERE period='{period}' AND crosses_capitol_hill=1"
        c.execute(query)
        rows = c.fetchall()
        
        cat_data = {
            "Overflights": {"trajs": [], "color": "#aa33ff", "title": "Overflights (Control)"},
...
plot_order = ["Overflights", "Arrivals26", "Arrivals8", "Departures"]
...
for i, period in enumerate(['aug2025', 'aug2026']):
    for j, cat in enumerate(plot_order):
        ...
        color = pdata["color"]
        alpha = 0.3 if len(pdata["trajs"]) < 500 else 0.1
        for pts in pdata["trajs"]:
            lats = [p[0] for p in pts]
            lons = [p[1] for p in pts]
            ax.plot(lons, lats, color=color, alpha=alpha, lw=1.2, zorder=5)
        ...
        ax.text(0.05, 0.05, f"N={count}", transform=ax.transAxes, color="white", fontsize=14, fontweight="bold", zorder=20)

plt.suptitle("Figure 9e: Structural Shift of Traffic Directly Crossing Capitol Hill (Aug 2025 vs 2026)", color="black", fontweight="bold", fontsize=14)
plt.tight_layout(rect=[0, 0, 1, 0.96])
plt.savefig(Path("figures") / "fig9e_ch_traffic.png", dpi=300)
plt.close()
